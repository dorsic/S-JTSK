...
class Krovak(NamedTuple):
    ...
    def plh_to_xy(plh):
        ...
        """
        phi, lam, h = plh
        dV = Krovak.alpha * (Krovak.lamKP - (lam+Krovak.lamFG))
        U = 2 * (np.arctan(Krovak.k * np.power(np.tan(phi/2+Krovak.ff), Krovak.alpha) * np.power((1+Krovak.e*np.sin(phi))/(1-Krovak.e*np.sin(phi)) ,-Krovak.alpha*Krovak.e/2)) - Krovak.ff)
        S = np.arcsin(np.cos(Krovak.a)*np.sin(U) + np.sin(Krovak.a)*np.cos(U)*np.cos(dV))
        D = np.arcsin(np.cos(U)*np.sin(dV)/np.cos(S))
        R0 = Krovak.kc * (Krovak.a_Bessel1841*np.sqrt(1-Krovak.e**2))/(1 - Krovak.e**2*np.sin(Krovak.phi0)**2) * 1/np.tan(Krovak.S0)
        Dc = D * np.sin(Krovak.S0)
        R = R0 * np.power(np.tan(Krovak.S0/2+Krovak.ff), np.sin(Krovak.S0)) * np.power(1/np.tan(S/2+Krovak.ff), np.sin(Krovak.S0))
        y = R * np.sin(Dc)
        x = R * np.cos(Dc)
        return x, y, 0
    ...
    def xyz_to_plh(xyz):
        ...
        Dc = np.arctan(y/x)
        R0 = Krovak.kc * (Krovak.a_Bessel1841*np.sqrt(1-Krovak.e**2))/(1 - Krovak.e**2 * np.sin(Krovak.phi0)**2) * 1/np.tan(Krovak.S0)
        S = 2 * (np.arctan(np.power(R0/R, 1/np.sin(Krovak.S0)) * np.tan(Krovak.S0/2+Krovak.ff)) - Krovak.ff)
        D = Dc/np.sin(Krovak.S0)
        U = np.arcsin(np.cos(Krovak.a)*np.sin(S) - np.sin(Krovak.a)*np.cos(S)*np.cos(D))
        dV = np.arcsin(np.cos(S)*np.sin(D)/np.cos(U))
        lam = (Krovak.lamKP - Krovak.lamFG) - dV/Krovak.alpha
        phi = U
        diff = 1
        while diff > 10E-12:
            phi1 = 2 * (np.arctan(np.power(Krovak.k, -1/Krovak.alpha) * np.power(np.tan(U/2+Krovak.ff), 1/Krovak.alpha) * np.power((1+Krovak.e*np.sin(phi))/(1-Krovak.e*np.sin(phi)), Krovak.e/2) ) - Krovak.ff)
            diff = abs(phi - phi1)
            phi = phi1
...
class Sjtsk(object):
    ...
    @staticmethod
    def etrs_sjtsk(phi, lam, hei, params=ETRF2000_JTSK03):
        """
    ...
    @staticmethod
    def sjtsk_etrs(x, y, z, params=JTSKO3_ETRF2000):
        """
        2 MATEMATICKÁ DEFINÍCIA VZŤAHU MEDZI ETRS89 (ETRF2000) A S-JTSK (JTSK03) [EPSG::8365 a EPSG::8367]
        """
...
def residuals_sjtsk_etrs(params, *args, **kwargs):
    ...
    for i in range(0, len(data)):
        etrs_plh = Sjtsk.sjtsk_etrs(data[i][4], data[i][3], 0, params)
...
def solve():
    ...
    opt = scipy.optimize.least_squares(residuals_sjtsk_etrs, Sjtsk.JTSKO3_ETRF2000, 
        gtol=None, xtol=1e-12, 
        args=data)
    print(opt)
...
def param_estimation():
    #solve() #JTSK->Etrs
    # solve 1 gtol=None (nfev: 20, njev: 14, cost: 6.42e-20, optimality: 9.84e-17)
    # [0.3325250810121545, 195.2759198197554, 842.8662883789235, -5.8969044470012175, -23.20799753791021, -0.09133061068530453, 0.00032697908030708414]
    # solve 2 gtol=None, xtol=1e-12 (nfev: 32, njev: 19, cost: 6.42e-20, optimality: 2.13e-18)
    # [0.33252508356015575, 195.27591982297898, 842.866288379478, -5.8969043491251485, -23.20799750236642, -0.09133065022694893, 0.0003268941649895157]
    # solve 3_0, SCALE = 1.0, gtol=None, xtol=1e-12 (nfev: 77, njev: 67, cost: 6.91e-20, optimality: 1.79e-22)
    # [7.1603252771037225, 108.85270566614534, 852.939185258333, -3.631911272440402, -23.45737335691424, -1.864031836198327, 0.0]
    data = read_data()
    params = [7.1603252771037225, 108.85270566614534, 852.939185258333, -3.631911272440402, -23.45737335691424, -1.864031836198327, 0.0]
    proj = sjtsk_etrs(params, data)
    np.savetxt("/test.csv", proj, delimiter=',')
